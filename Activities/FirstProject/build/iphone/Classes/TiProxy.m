...
//Rare exceptions to indicate a bug in the _firstproject code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_firstproject = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_firstproject]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.firstproject.module"
