...
//Rare exceptions to indicate a bug in the _testproject code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_testproject = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_testproject]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.testproject.module"
